...
function Graphs() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await fetch("http://localhost:8000/download-graph-images", {
        method: "POST",
        body: formData,
      });
      if (response.ok) {
        ...
        link.href = url;
        link.setAttribute("download", "graph_images.zip"); // Custom filename
    ...
    <Box>
      ...
      <Stack spacing={2}>
        ...
        <Button
          ...
          disabled={loading}
        >
          {loading ? <CircularProgress size={24} /> : "Download Graph Images"}
        </Button>
