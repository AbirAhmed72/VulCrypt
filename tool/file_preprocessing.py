...
def preprocess(java_path, extracted_folder):
    with open(java_path, "r") as f:
        f_code = f.readlines()
    code_list = remove_empty(f_code)
    code_list = replace_within_quotes(code_list, 30)
    code_list = remove_function(code_list)
    code = removeJADXwarn(code_list)
    code = else_catch(code)
    
    java_file = java_path.split('/')[-1]
    java_file_path = f'{extracted_folder}/{java_file}'
    print(f"in file preprocessing inside preprocess: java_file_path is: {java_file_path}")
